...
test('test game board to receive an out of range attack', () => {
    const gameBoard = makeGameBoard(5);
    expect(gameBoard.receiveAttack(5, 4)).toBe(constants.ATTACK_INVALID);
});

test('test game board to receive attacks at the same position', () => {
    const gameBoard = makeGameBoard(5);
    gameBoard.receiveAttack(2, 4);
    expect(gameBoard.receiveAttack(2, 4)).toBe(constants.ATTACK_INVALID);
});
